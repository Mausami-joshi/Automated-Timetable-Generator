...
<div class="card shadow-sm border-0 col-md-8 mx-auto">
    <div class="card-header bg-white py-3">
        <h5 class="mb-0 text-primary">Subject Details</h5>
    </div>
    <div class="card-body">
        <form action="{{ route('subjects.store') }}" method="POST">
            ...
            <div class="mb-3">
                ...
                <input type="text" name="subject_name" class="form-control @error('subject_name') is-invalid @enderror" value="{{ old('subject_name') }}" required placeholder="e.g. Data Structures & Algorithms">
            ...
            <div class="mb-3">
                ...
                <select name="term" id="term" class="form-select @error('term') is-invalid @enderror" required>
                    <option value="odd" {{ old('term', 'odd') == 'odd' ? 'selected' : '' }}>Odd (1,3,5,7)</option>
                    <option value="even" {{ old('term') == 'even' ? 'selected' : '' }}>Even (2,4,6,8)</option>
                </select>
            ...
            <div class="mb-3">
                ...
                <input type="number" name="hours_per_week" class="form-control @error('hours_per_week') is-invalid @enderror" value="{{ old('hours_per_week', 3) }}" min="1" max="40" required>
                @error('hours_per_week') <div class="invalid-feedback">{{ $message }}</div> @enderror
                <div class="form-text">For labs, use even hours (2,4,6...). Labs are scheduled in 2-hour sessions.</div>
            </div>

            <div class="mb-4 form-check">
                <input class="form-check-input" type="checkbox" name="is_lab" id="is_lab" value="1" {{ old('is_lab') ? 'checked' : '' }}>
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        ...
        function filterCourses() {
            ...
            // reset course selection if currently invalid
        ...
        // initial filter
        filterSemesterOptions();
        // if a course was pre-selected (old input), infer semester and filter
