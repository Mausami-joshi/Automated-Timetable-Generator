...
@section('title', 'Add Faculty Availability')

@section('content')
<div class="card shadow-sm border-0 col-md-8 mx-auto">
    <div class="card-header bg-white py-3">
        <h5 class="mb-0 text-primary">Availability Details</h5>
    </div>
    <div class="card-body">
            ...
            <div class="mb-3">
                ...
                <select name="teacher_id" class="form-select @error('teacher_id') is-invalid @enderror" required>
                    ...
                    @foreach($teachers as $teacher)
                        <option value="{{ $teacher->id }}" {{ old('teacher_id') == $teacher->id ? 'selected' : '' }}>{{ $teacher->name }} ({{ $teacher->department }})</option>
                    @endforeach
                </select>
